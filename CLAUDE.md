...
- **Contacto de sponsors:** el formulario envía a `info@excellenceproducciones.com` mediante un **endpoint PHP propio en Hostinger** (`public/contact.php`) con **PHPMailer** sobre el SMTP del hosting (sin servicios de terceros). El sitio se mantiene estático; el PHP viaja dentro de `public/` → `dist/`. Credenciales en `public/mail-config.php` (no versionado). Ver [FORMULARIO-CONTACTO.md](FORMULARIO-CONTACTO.md).
