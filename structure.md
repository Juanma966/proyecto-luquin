...
| :--- | :--- | :--- | :--- |
| [Header.astro](src/components/Header.astro) | Header | `/` | Barra superior sticky: logo placeholder + CTA "Comprar entradas". |
| [Hero.astro](src/components/Hero.astro) | Hero | `/` | Título, fecha/lugar, CTA principal (entradas) + secundario (→ `#sobre`). |
| [AboutEvent.astro](src/components/AboutEvent.astro) | Sobre el evento (`#sobre`) | `/` | Introducción + 3 highlights placeholder. |
...
| [BecomeSponsor.astro](src/components/BecomeSponsor.astro) | Ser sponsor (`#ser-sponsor`) | `/` | Intro leve + imágenes placeholder + botón → `/sponsors`. |
...
```
Header → Hero → Sobre el evento → Entradas → Ser sponsor → Footer
```
